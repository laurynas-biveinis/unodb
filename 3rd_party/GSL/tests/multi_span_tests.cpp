...
GSL_SUPPRESS(con .4) // NO-FORMAT: attribute
GSL_SUPPRESS(
    bounds .4) // NO-FORMAT: attribute // false positive, checker does not recognize multi_span yet
GSL_SUPPRESS(r .11) // NO-FORMAT: attribute
GSL_SUPPRESS(r .3)  // NO-FORMAT: attribute
TEST_CASE("md_access")
{
...
TEST_CASE("iterator")
{
...

    {
        ...
        for (std::size_t i = 0; i < 4; ++i) { CHECK(a[i] == 0); }
    }

    {
        multi_span<int, dynamic_range> av = a;
        for (auto& n : av) { n = 1; }
        for (std::size_t i = 0; i < 4; ++i) { CHECK(a[i] == 1); }
    }
}
