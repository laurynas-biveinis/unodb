 ...
 protected:
  ...
  void do_add_value(std::uint64_t value, double &double_value, double &delta,
    ...
    while (true) {
      ...
      if (UNODB_DETAIL_LIKELY(count_and_mean.dword.compare_exchange_weak(
              old_count_and_mean.dword, new_count_and_mean.dword,
...
#else
    __extension__ std::atomic<__int128> dword;
#endif
    struct {
...
class [[nodiscard]] lock_free_max_variance_stats : public lock_free_stats {
 public:
  void add_value(std::uint64_t value) noexcept {
    ...
    auto current_msq = msq.load(std::memory_order_relaxed);
    while (true) {
      const auto new_msq = current_msq + msq_delta;
      if (UNODB_DETAIL_LIKELY(msq.compare_exchange_weak(
              current_msq, new_msq, std::memory_order_release,
    ...
    while (true) {
      ...
      if (UNODB_DETAIL_LIKELY(max.compare_exchange_weak(
              current_max, value, std::memory_order_release,
              std::memory_order_relaxed)))
        break;
    }
  }
