...
void olc_db::iterator::dump(std::ostream &os) const {
  ...
  while (!tmp.empty()) {
    ...
    os << "iter::stack:: level = " << level << ", key_byte=0x" << std::hex
       ...
       << std::setw(2) << static_cast<uint64_t>(e.child_index) << std::dec
       << ", version=";
...
olc_db::iterator &olc_db::iterator::seek(const detail::art_key search_key,
...
bool olc_db::iterator::try_seek(const detail::art_key &search_key, bool &match,
                                bool fwd) noexcept {
